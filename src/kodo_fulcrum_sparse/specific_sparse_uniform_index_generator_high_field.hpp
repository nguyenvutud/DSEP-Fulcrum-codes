...
class specific_sparse_uniform_index_generator_high_field : public SuperCoder
{
public:
...
public:
    ...
    /// Constructor
    specific_sparse_uniform_index_generator_high_field() :
...
//            	if(index >= SuperCoder::symbols() - SuperCoder::expansion())
            	if(index >= SuperCoder::symbols() - 2)
            	    SuperCoder::set_coefficient_value(coefficients, index, 1U);
            	else
            	   	SuperCoder::set_coefficient_value(coefficients, index, coefficient);
            }

...
#include <kodo_fulcrum_sparse/systematic_coefficient_mapper_high_field.hpp>
//#include <kodo_fulcrum/fulcrum_info.hpp>
#include <kodo_fulcrum_sparse/fulcrum_sparse_info.hpp>
...
#include <kodo_fulcrum_sparse/fulcrum_payload_decoder_counter_high_field.hpp>
...
>
class fulcrum_SS_high_field_combined_decoder : public
    // Payload API
    kodo_core::nested_read_payload<
    kodo_core::nested_payload_size<
	kodo_fulcrum::stage_two_operation_counter<
    fulcrum_proxy_stack<kodo_core::proxy_args<>, fulcrum_payload_decoder_counter_high_field, //Added finite counter
    // Codec Header API
    ...
    kodo_core::elimination_decoder<InnerField,
    typename Features::template remove<kodo_core::is_storage_layers>>,
    kodo_core::basic_symbol_decoder_counter<InnerField,
    typename Features::template remove<kodo_core::is_storage_layers>>,
    trace_systematic_coefficient_mapper<
    kodo_core::find_enable_trace<Features>,
    kodo_fulcrum::systematic_coefficient_mapper_high_field<
...
//	kodo_core::uniform_generator<
    ...
    kodo_core::finite_field_layers<OuterField,
...
public:
    ...
    static_assert(!std::is_same<OuterField,fifi::prime2325>::value,
                  "The mapping between inner and outer code requires "
                  "that both are binary extension fields");

    // Define the nested factory type
      using factory = kodo_core::pool_factory<fulcrum_SS_high_field_combined_decoder>;

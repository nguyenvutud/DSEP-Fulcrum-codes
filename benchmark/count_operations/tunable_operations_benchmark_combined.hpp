...
class tunable_operations_benchmark_combined : public gauge::benchmark
{
public:
    ...
    void stop()
    {
		...
		m_counter_stage1 = kodo_core::operations_counter();
		m_counter_stage2 = kodo_core::operations_counter();
        if (type == "encoder")
        {
        ...
        {
		//m_counter_nested = kodo_core::operations_counter();
           m_counter_stage1 = m_decoder->stage_one_operations_counter(); //Using for measuring Combined codes
           m_counter_stage2 = m_decoder->stage_two_operations_counter(); //Using for measuring Combined codes
  ...
  void store_run(tables::table& results)
    {
        ...
        results.set_value("add", m_counter.m_add+m_counter_nested.m_add+m_counter_stage1.m_add+m_counter_stage2.m_add);

        if (!results.has_column("sub"))
            results.add_column("sub");

        results.set_value("sub", m_counter.m_subtract+m_counter_nested.m_subtract +m_counter_stage1.m_subtract+m_counter_stage2.m_subtract);

        if (!results.has_column("mul"))
            results.add_column("mul");

        results.set_value("mul", m_counter.m_multiply+m_counter_nested.m_multiply+m_counter_stage1.m_multiply+m_counter_stage2.m_multiply);

        if (!results.has_column("add_mul"))
            results.add_column("add_mul");

        results.set_value("add_mul",
                          m_counter.m_multiply_add + m_counter_nested.m_multiply_add +m_counter_stage1.m_multiply_add+m_counter_stage2.m_multiply_add);

        if (!results.has_column("sub_mul"))
            results.add_column("sub_mul");

        results.set_value("sub_mul",
                          m_counter.m_multiply_subtract + m_counter_nested.m_multiply_subtract+m_counter_stage1.m_multiply_subtract+m_counter_stage2.m_multiply_subtract);

        if (!results.has_column("invert"))
            results.add_column("invert");

        results.set_value("invert", m_counter.m_invert + m_counter_nested.m_invert +m_counter_stage1.m_invert+m_counter_stage2.m_invert);
    ...
    double calculate_region(uint32_t symbol, uint32_t num_regions, const uint32_t exp = 0)
	  {
		return (symbol) * (pow(2, num_regions) - 1)/ pow(2, num_regions);
	  }
    ...
    uint32_t get_operations()
    {
        ...
        sum_operations = m_counter.m_add+m_counter_nested.m_add+m_counter_stage1.m_add+m_counter_stage2.m_add +
        				 m_counter.m_subtract+m_counter_nested.m_subtract +m_counter_stage1.m_subtract+m_counter_stage2.m_subtract +
						 m_counter.m_multiply+m_counter_nested.m_multiply+m_counter_stage1.m_multiply+m_counter_stage2.m_multiply+
						 m_counter.m_multiply_add + m_counter_nested.m_multiply_add +m_counter_stage1.m_multiply_add+m_counter_stage2.m_multiply_add+
						 m_counter.m_multiply_subtract + m_counter_nested.m_multiply_subtract+m_counter_stage1.m_multiply_subtract+m_counter_stage2.m_multiply_subtract +
						 m_counter.m_invert + m_counter_nested.m_invert +m_counter_stage1.m_invert+m_counter_stage2.m_invert;
    ...
    void run_fulcrum()
     {
         ...
         gauge::config_set cs = get_current_configuration();
         uint32_t expansion = cs.get_value<uint32_t>("expansion");
         uint32_t symbol = cs.get_value<uint32_t>("symbols");
 ...
 //                std::cout<<"Symbol_used:"<<symbols_used<<std::endl;
    ...
    void run()
    {
        ...
        RUN
        {
			...
			size_cur_region = calculate_region(symbol - expansion, num_regions);

			while (!m_decoder->is_complete())
			{
				symbols_used++;

				if ((received_symbols > size_cur_region) && (cur_r < expansion))
				{
    ...
    void run_region_improvement()
     {
         ...
         RUN
         {
 			...
 			size_cur_region = std::min(double(symbol-expansion + cur_r), calculate_region(symbol, num_regions));
...
//						std::cout<<"current coded packets:"<<received_symbols<<std::endl;

					}
					num_regions += 1;
					size_cur_region = std::min(double(symbol-expansion + cur_r),
							calculate_region(symbol, num_regions));
					//std::cout<<"new region:"<<num_regions<<" with size:"<<size_cur_region<<std::endl;


				}
				if (received_symbols >= symbol)
    ...
    void run_new_evolution_tunable()
    {
        // Check with the result memory whether we already have
        // results for this configuration
        gauge::config_set cs = get_current_configuration();
        ...
        RUN
        {
			...
			while (!m_decoder->is_complete())
			{
				symbols_used++;
 			 	...
 			 	if (cur_r < expansion)
                    		{
						if (symbol - received_symbols <= std::max(threshold, expansion))
						{
							cur_r = expansion;

						}

						if (symbol - received_symbols - expansion <= threshold)
						{
							cur_r += 1;
...
protected:
    ...
    kodo_core::operations_counter m_counter_stage1;
    kodo_core::operations_counter m_counter_stage2;
